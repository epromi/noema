...
describe('agents session classification', () => {
	it('sessionAgeMs prefers ageMs over updatedAt', () => {
		const session: Session = { key: 'a', ageMs: 120_000, updatedAt: NOW - 999_999 };
...
describe('agents', () => {
	it('getAgents returns agent cards via provider', async () => {
		...
		expect(data.agents.find((a) => a.id === 'alfred')?.sessionStatus).toBe('active');
		expect(data.agents.find((a) => a.id === 'alfred')?.lastActive).toBe('just now');
