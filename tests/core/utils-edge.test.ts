...
describe("utils edge cases", () => {
  describe("countFiles", () => {
    ...
    it("handles directories recursively", async () => {
      const readDir = vi.fn(async (dir: string) => {
        ...
        if (dir === "/test/subdir") {
          return [{ name: "deep.ts", isDirectory: false, mtimeMs: Date.now() }];
        }
        return [];
      });
  ...
  describe("classifyCronGroup", () => {
    it("classifies hourly schedules as SPANNING", () => {
      ...
      expect(classifyCronGroup({ kind: "every", everyMs: 3600000 })).toBe(
        "SPANNING",
      );
    });
  ...
  describe("formatSchedule", () => {
    ...
    it("formats every-ms schedules as hours", () => {
      expect(formatSchedule({ kind: "every", everyMs: 7200000 })).toBe(
        "every 2h",
      );
      expect(formatSchedule({ kind: "every", everyMs: 900000 })).toBe(
        "every 15m",
      );
      expect(formatSchedule({ kind: "every", everyMs: 60000 })).toBe(
        "every 1m",
      );
    });
