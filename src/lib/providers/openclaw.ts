...
function workspaceRoot(config?: OpenClawConfig): string {
	return config?.workspace ?? process.env.WORKSPACE ?? join(homedir(), '.openclaw', 'workspace');
}
...
export function createOpenClawProviders(config: OpenClawConfig = {}): AllProviders {
	...
	const cron: CronProvider = {
		async listCrons(): Promise<CronJob[]> {
			const raw = await runCmd('openclaw', ['cron', 'list', '--json'], { timeoutMs });
		...
		async getCronStatus(): Promise<CronStatus> {
			const raw = await runCmd('openclaw', ['cron', 'status', '--json'], { timeoutMs });
		...
		async getCronRuns(jobId: string): Promise<CronRun[]> {
			...
			const parsed = parseJson<{ runs?: CronRun[] } | CronRun[]>(raw, { runs: [] });
	...
	const session: SessionProvider = {
		async listSessions(filter: SessionFilter = {}): Promise<Session[]> {
			...
			if (filter.activeMinutes != null) args.push('--active', String(filter.activeMinutes));
		...
		async getHistory(sessionKey: string): Promise<Message[]> {
			...
			const parsed = parseJson<{ messages?: Message[]; turns?: Message[] }>(raw, {});
	...
	const agent: AgentProvider = {
		async listSubagents(): Promise<Subagent[]> {
			...
			const parsed = parseJson<{ tasks?: Subagent[] } | Subagent[]>(raw, { tasks: [] });
