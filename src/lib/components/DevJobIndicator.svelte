<script lang="ts">
	...
	const countdown = $derived(formatDevJobCountdown(status.nextMs, !!status.error, now));
	...
	function onDragMove(event: MouseEvent | TouchEvent) {
		...
		event.preventDefault();
		const { x, y } = pointerClientXY(event);
		setPosition(dragStartLeft + (x - dragStartX), dragStartTop + (y - dragStartY));
	}
...
>
	...
	<div class="dji-row">
		...
		<span class="dji-val" class:queue-active={status.queue > 0}>{status.queue}</span>
	</div>
