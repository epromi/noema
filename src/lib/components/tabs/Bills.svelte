<script lang="ts">
	...
	function parseBillRow(bill: BillEntry): BillRow {
		...
		const dueMatch = desc.match(/(\d{4}-\d{2}-\d{2}|\d{2}\/\d{2}|\d{1,2}\.\s*\w+)/);
		...
		return {
			id: bill.id,
			name: desc.replace(/https?:\/\/\S+/g, '').replace(/\d[\d\s.,]*\s*Ft/i, '').trim() || desc,
			amount,
			due,
			status,
			link,
	...
	function escalationLevel(loop: OpenLoop): { icon: string; className: string } {
		...
		if (loop.severity === 'yellow') return { icon: '🚩', className: 'esc-flag' };
				...
				<tbody>
					{#each billRows as row (row.id)}
						...
						<tr class={billDueClass(row)}>
							...
							<td>
								<span class="status-badge {billStatusClass(row.status)}">{row.status}</span>
							</td>
							<td>
								{#if row.link}
