...
<section class="h1-tab">
	...
	<div class="metrics-bar">
		...
		<div class="metric-card">
			<div class="metric-value">{h1.error ? 'N/A' : na(h1.stats.reputation)}</div>
				...
				<tbody>
					{#each programs as program (program.id)}
						<tr class:primary-row={program.primary}>
							<td class="col-name">
								{#if program.primary}<span class="primary-badge">⭐ PRIMARY</span>{/if}
								{program.name}
							</td>
							<td class="mono">{program.handle}</td>
							<td>{program.programType ?? (program.offersBounties ? 'BBP' : 'Non-BBP')}</td>
							<td class="muted">{program.scopeType ?? '—'}</td>
							<td>{program.bountyRange ?? (program.offersBounties ? 'BBP' : 'Non-BBP')}</td>
