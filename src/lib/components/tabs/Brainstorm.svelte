...
<section class="brainstorm-tab" aria-label="Brainstorming action tracker">
  ...
  {:else if sectionsWithItems.length === 0}
    <div class="card wide empty-card">
      <p>
        No brainstorming items tracked. Run the brainstorming cron to populate.
      </p>
    </div>
  {:else}
    <div class="grid">
        ...
        <div class="card section-card" style:border-left-color={section.color}>
          ...
          {#each section.items as item (item.id)}
            <div
              ...
              style:background={!item.done && section.bgColor
                ? section.bgColor
                : undefined}
            >
              ...
              <div class="item-body">
                ...
                {#if item.source || item.age}
                  <span class="item-meta">
                    {item.source}{#if item.source && item.age}
                      ·
                    {/if}{item.age}
                  </span>
                {/if}
  ...
  <p class="legend">
    Source: memory/brainstorming/action-tracker.md · Items not seen here are
    invisible → they rot.
  </p>
</section>
