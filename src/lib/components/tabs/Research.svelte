<script lang="ts">
	...
	function ottoIcon(status: ResearchData['ottoRuns'][number]['status']): string {
		if (status === 'ok') return '✅';
	...
	{:else}
		<div class="card research-summary">
			<div class="badges">
				...
				<span class="badge badge-purple">💡 {Math.max(0, research.totalFiles - research.recentFiles)} corpus</span>
			</div>

			{#if research.proposals.length === 0}
				<p class="empty-inline">No proposals yet — Research cron generates them at 01:00.</p>
			{:else}
				<div class="proposals">
				...
				{#each recentOttoRuns as run (run.date)}
					...
					>
						...
						{#each run.steps as step (`${run.date}-${step.label}`)}
							<div class="otto-step">{step.status === 'ok' ? '✅' : '⬜'} {step.label}</div>
						{/each}
					</div>
				{/each}
			</div>
