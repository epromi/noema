<script lang="ts">
	...
	const CRON_DESCRIPTIONS: Record<string, string> = {
		'Autoresearch Orchestrator': 'Viktor benchmark loop — blind repo audits, recall stats',
		'Nightly Hacker Intel': 'Albert — CVE/exploit intelligence, H1 program scanning',
		'Autoexecutor A (02:00)': 'Action tracker AUTOEXECUTE items (Flash) — off-peak 00-03',
		'Otto Core Analysis': 'Memory audit + agent status + KG consistency check',
		'Cortex Pipeline Optimizer': 'System optimization proposals for Viktor pipeline',
		'Edwin Morning Study': 'Capability research: news, self-improvement',
		'Otto Compilation': 'Nightly review → Telegram (bilingual metrics)',
		'Nightly Staff Review (Core)': 'Memory audit + agent status + KG consistency check',
		'Nightly Staff Review (Compile)': 'Nightly review → Telegram (bilingual metrics)',
		'Porter Hourly Triage': 'Gmail scan: categorize, bill detection → calendar+tasks',
		'Autoexecutor B (07:00)': 'Action tracker AUTOEXECUTE items (Flash) — off-peak 06-08',
		...
		'Idle Brainstorming Trigger': 'Research-backed action item generation (Flash)',
		'Dashboard Refresh (2h)': 'Dashboard HTML regeneration from live data',
		'Dashboard Research & Auto-Fix': 'Dashboard analysis + web research + auto-fix',
	...
	const GROUP_ORDER: CronGroup[] = ['NIGHT', 'MORNING', 'SPANNING', 'DAYTIME', 'EVENING'];
		...
		crons.crons
			.map((cron) => {
				...
				return {
					...
					color: cron.lastResult === 'ok' ? 'var(--green)' : cron.lastResult === 'error' ? 'var(--red)' : 'var(--yellow)'
				};
			})
	...
	function parseScheduleMinutes(sched: string): number | null {
		if (!sched || sched === '—' || sched === 'auto' || sched === 'unknown') return null;
	...
	function getActionState(itemId: string, action: DashboardActionType): ActionBtnState {
	...
	function kanbanColumns(): { key: keyof ActionQueueData; title: string; items: ActionQueueItem[] }[] {
		return [
		...
		{:else}
			<div class="timeline">
				{#each research.ottoRuns as run (run.date)}
					<div class="tl-item" class:tl-warn={run.status === 'warn'} class:tl-err={run.status === 'err'}>
						<div class="tl-head">
							{ottoIcon(run.status)} {run.title}
						...
						{#each run.steps as step (`${run.date}-${step.label}`)}
							<div class="tl-sub">{step.status === 'ok' ? '✅' : '⬜'} {step.label}</div>
						{/each}
					</div>
				{/each}
			</div>
	...
	{:else}
		<div class="cron-timeline">
			{#each Array.from({ length: 13 }) as _, i (i)}
				...
				<div class="tl-label" style:left="{left}%">{String(hour).padStart(2, '0')}:00</div>
			{/each}
			...
			{:else}
				<div class="proposal-list">
					{#each research.proposals as proposal (proposal.id)}
						...
						>
							<span class="proposal-text">{proposal.priority} {proposal.finding}</span>
							...
							{:else}
								<ImplementButton
									...
									onImplement={() => onImplement?.(proposal.id, proposal.finding)}
