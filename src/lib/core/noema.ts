...
export async function getNoema(providers?: AllProviders): Promise<NoemaData> {
	...
	try {
		...
		const [genLines, relayLines, procLines, changelog, cronJobs] = await Promise.all([
		...
		const cronHealthy = NOEMA_CRON_IDS.filter((prefix) =>
			cronJobs.some((j) => j.id.startsWith(prefix) && (j.state?.lastRunStatus ?? j.status) === 'ok')
		).length;

		let activeProposals = 0;
		try {
			const indexMd = await readFile(join(noemaDir, 'dev', 'packages', 'INDEX.md'), 'utf8');
