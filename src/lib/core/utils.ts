...
export function parseMarkdownTable(content: string, headerMarker: string): string[][] {
...
export function formatSchedule(schedule: unknown): string {
	...
	if (schedule && typeof schedule === 'object') {
		...
		if (s.kind === 'every' && typeof s.everyMs === 'number') {
			...
			return hours >= 1 ? `every ${hours}h` : `every ${Math.round(s.everyMs / 60_000)}m`;
		}
	...
	async function walk(dir: string, depth: number): Promise<void> {
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory) {
				...
				total++;
				if (entry.mtimeMs != null && Date.now() - entry.mtimeMs < weekMs) recent++;
			}
		}
	}
